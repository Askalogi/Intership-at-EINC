...
def create_dataset(num_samples : int, size = 16):
    ...
    #we create a blank labels list to add all the labels in order oftheir creation
    labels = []
    ...
    for i in range(num_samples):
        ...
        #use the funciton defined above
        ...
        #create the file path and the names tha these images will have 
        file_path = os.path.join(dataset_path, f"{i+1}_{label}.png")

        #save them there
        img.save(file_path)

        #and append their labels in the previously empty list
        labels.append((label))

    #We transform the list to a dataframe and later create a csv file so its easier to use 
    df = pd.DataFrame(labels, columns=["label"])
    df.to_csv(os.path.join("./custom_dataset/labels.csv"), index=False)


#!TESTING

create_dataset(500, 16)
